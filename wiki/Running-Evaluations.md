...
| `YAM_single_arm` | `yam.usd` (YAMLab arm, bare) | 6 | stock `JointController`, YAMLab `high_pd` gains | see below |
| `YAM_single_arm_base_pd_control` | `yam.usd` | 6 | stock `JointController`, YAMLab `base` gains | |
...
| `YAM_molmoact2_rest_pose` | `yam_bimanual.usd` | 2 x 6 | as `YAM_bimanual` | MolmoAct2-like top camera 0.30 m ahead / 1.26 m above the arm bases looking straight down (15 mm), wrist cameras at YAMLab's 25 deg calibration (`wrist_camera_pose`); for `--model_type openpi_yam` |
| `YAM_molmoact2` | `yam_bimanual.usd` | 2 x 6 | as `YAM_bimanual` | `YAM_molmoact2_rest_pose` + arms start in MolmoAct2's mid-episode working pose (out over the table); **the configuration in which pi05-yam-molmoact2 acts** -- see [YAM-Port-Status](YAM-Port-Status) |
| `YAM_ABC` | `yam_crank_bimanual.usd` (the same workstation with I2RT's crank gripper) | 2 x 6 | as `YAM_bimanual` but YAMLab's `high_pd` arm gains (the aligned set is the row below) | ABC-project gripper: inverted finger sign, steeper wrist camera, ABC home pose; see below |
| `YAM_ABC_aligned_pd_control` | `yam_crank_bimanual.usd` | 2 x 6 | stock `JointController`, `abc_aligned` gains (kp 160 / kd 20 on every arm joint) | Fitted by replaying ABC's real put-bottles episodes (`scripts/yam_pd_search.py`): a uniform ~93 ms lag, RMSE 0.024 rad vs 0.028 for `high_pd`; see [YAM-Port-Status](YAM-Port-Status) |
...
**YAM_single_arm.** Ported from [YAMLab](https://github.com/ARISE-Initiative/yamlab): the spec is
...
* **Start state**: the arms start at the MolmoAct2-BimanualYAM episode start pose (median first frame over 66
  ...
  dataset starts open). DROID's warm-up still ends closed. `YAM_ABC` keeps ABC's `home` pose.
